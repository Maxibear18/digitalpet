...
let appPath = process.cwd(); // Fallback to current working directory
...
function resolveSpritePath(relativePath) {
    ...
    // Resolve from app root
    const absolutePath = path.resolve(appPath, cleanPath);
    // Convert to file:// URL format (works in Electron)
    const normalizedPath = absolutePath.replace(/\\/g, '/');
    // On Windows, we need file:///C:/path format
